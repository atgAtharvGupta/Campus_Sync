...
def specifications(request,spe_id):
    allproduct = Software_info.objects.filter(pk=spe_id)
    context = {'allproduct': allproduct}
    return render(request,'Software/Specifications.html',context)

def order_detail(request,ord_id):
    allproduct = Software_info.objects.filter(pk=ord_id)
    context = {'allproduct': allproduct}
    return render(request,'Software/Order_detail.html',context)

def purchase_detail(request,pur_id):
    allproduct = Software_info.objects.filter(pk=pur_id)
    context = {'allproduct': allproduct}
    return render(request,'Software/Purchase_detail.html',context)
...
def add(request):
    ...
    if request.method == 'POST':
       ...
       if PurD >= ExpD:
          ...
          software_Add = Software_info(Soft_id=Soft_id,SName=SName,Users=Users,PurD=PurD,ExpD=ExpD,PurForSub=PurForSub,ind_pri=ind_pri,auth=auth,Tot_users=Tot_users,Pur_from=Pur_from,tot_pri=tot_pri,order_ref=order_ref, order_date= order_date,in_no=in_no,spec=spec)
          software_Add.save()
